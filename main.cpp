...
int main()
{
    ...
    else
    {
        ...
        while(!feof(a))
        {
            fscanf(a,"%d,%[^,],%[^,],%d,%[^,],%[^,],%d,%d,%d,%d,%d,",
                   &s[i].number,s[i].name,s[i].sex,&s[i].birth,s[i].major,s[i].telephone,&s[i].judge1,&s[i].judge2,&s[i].judge3,&s[i].judge4,&s[i].judge5);
            i++;
        }
        j=i;
        for(i=0;i<j;i++)
    ...
    b=fopen("/Users/S20171105117/Desktop/studentdada2.0.csv","w");
              fprintf(b,"number,name,sex,birth,major,telephone,judge1,judge2,judge3,judge4,judge5,score\n");
              j=i;
              i=0;
              while(i<j)
              {
                  fprintf(b,"%d,%s,%s,%d,%s,%s,%d,%d,%d,%d,%d,%d\n",s[i].number,s[i].name,s[i].sex,s[i].birth,
                          s[i].major,s[i].telephone,s[i].judge1,s[i].judge2,s[i].judge3,s[i].judge4,s[i].judge5,s[i].score);
                  i++;
              }
              fclose(b);             
              return 0;
